...
int getOpenCountLastHour(int door) {
  ...
  for (int i = 0; i < doorOpenTimestampsSize[door];
       i++) { // Go through all the timestamps
    if (currentTime - doorOpenTimestamps[door][i] <=
        tarkasteluaika) { // If inside 1h,
      doorOpenTimestamps[door][count++] =
          doorOpenTimestamps[door][i]; // move it to the next index, count++
    }
  }
...
void openDoor(int door) {
  ...
  doorOpenCount[door]++;
  doorOpenTimestamps[door][doorOpenTimestampsSize[door]++] =
      millis(); // Save the timestamp and increase the table size
  Serial.print(door);
...
void checkHall(int sensor) {
  ...
  if (hallState == HIGH && millis() - hallHighStartTime[sensor] >= openDelay &&
      !isDoorOpen[sensor]) {
    openDoor(sensor);
  }
